...
describe('Post Likes (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    // Регистрируем пользователя для тестирования лайков
    const userData = {
    ...
    // Подтверждаем регистрацию (если требуется)
    // В реальном приложении это может потребовать получения кода из email
